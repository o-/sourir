    ...
    | Scope scope ->
      begin match annotations.(pc) with
        ...
        | Some (AtLeastScope vars) ->
          if not (VarSet.subset vars scope)
          then raise (UndeclaredVariable (VarSet.diff vars scope, pc))
        | Some (ExactScope vars) ->
          if not (VarSet.subset vars scope)
          then raise (UndeclaredVariable (VarSet.diff vars scope, pc));
          if not (VarSet.subset scope vars)
          then raise (ExtraneousVariable (VarSet.diff scope vars, pc))
      end
  in
...
let explain_incompatible_scope outchan s1 s2 pc =
  ...
  let print_vars buf vars =
    let print_var buf var = Printf.bprintf buf "%s" var in
